...
impl Editor {
    ...
    fn match_paren_expected_char(&self) -> Option<char> {
        ...
        if let Some(text) = self.grid_cell_text(cursor_grid, cursor_row, cursor_column) {
            return Self::match_paren_expected_char_from_text(&text);
        }
    ...
    #[cfg(target_os = "macos")]
    fn match_paren_text_at_cursor(&self) -> Option<String> {
        let grid = self.cursor.parent_window_id;
        let row = self.cursor.grid_position.1;
        let column = self.cursor.grid_position.0;
        let text = self.grid_cell_text(grid, row, column)?;
        if Self::match_paren_expected_char_from_text(&text).is_some() {
            Some(text)
        } else {
            None
        }
    ...
    fn maybe_flash_match_paren_from_cache(&mut self) {
        ...
        if self.match_paren_text_at_cursor().is_none() {
            return;
        }
